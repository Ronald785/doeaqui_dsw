const { readCenter, readCenterInventory, createCenter, updateCenter, createCenterInventory, updateCenterInventory, deleteCenter } = require("../models/point");
...
async function editInventory(server, req, res) {
    ...
    res.render("point/inventory", { inventorys: arrayInventory, id, state: stateInventory });
}
...
async function putInventory(server, req, res) {
    const { id } = req.params;
    const { jsonInventory, stateInventory } = req.body;

    let json = JSON.parse(jsonInventory.replaceAll("'", '"'));
    let data = { idCenter: id, data: json};
    
    if(stateInventory == "true") {
        await updateCenterInventory(json, id);
    }
    else {
        await createCenterInventory(data);
    }
